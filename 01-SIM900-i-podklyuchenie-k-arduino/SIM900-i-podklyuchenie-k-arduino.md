...
3) При выборе последовательного порта убедитесь, что ***перемычки подключены***, как показано на рисунке ниже (***через 7 и 8 пины, а также 9 для программного включения платы***), для использования программного обеспечения. 

Как правило, перемычки установлены ***для работы с FTDI через 0 и 1 пины***.
...
### [Автоматическое включение модуля](https://wiki.alashed.kz/arduino/rnt/sim900-gsm-gprs-shield-arduino/index.html)

Вместо ручного нажатия кнопки «power» для включения модуля, вы можете автоматически включить модуль программно:

- сначала вам нужно припаять соединение R13 на модуле, как показано на рисунке ниже — выделено красным;

![Пайка соединения R13 на SIM900 GSM Shield для автоматического включения](R13.jpg)

- затем подключить D9 на модуле к пину D9 Arduino;

- добавьте следующий фрагмент кода. Это эквивалент нажатия кнопки «power» на модуле.
...
1.  Поддерживает ли ваше местоположение сеть 2G?
SIM900 - это сетевое устройство 2G. 

2. Найдите список AT-команд и узнайте больше о качестве сигнала, подключениях и т. д. Вы можете начать с проверки качества сигнала с помощью команды, например 
...
и отправить вывод обратно в последовательный порт. 
...
5. В примерах для связи с SIM900 через последовательный порт используется библиотека SoftwareSerial. Нет необходимости подключать другую библиотеку.
